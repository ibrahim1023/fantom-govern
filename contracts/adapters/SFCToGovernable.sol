...
interface SFC {
    function delegations(address _from, uint256 _toStakerID)
        external
        view
        returns (
    ...
    function stakers(uint256 _stakerID)
        external
        view
        returns (
            uint256 status,
...
contract SFCToGovernable is Governable {
        ...
        returns (uint256)
    {
        ...
        (
            ...
            ,
            ,
            uint256 toDeactivatedEpoch,
            ,
            uint256 toStakeAmount,
            ,
            ,
            ,
            address sfcAddress
        ) = sfc.stakers(toStakerID);
        ...
        if (from == to) {
            ...
            (, , uint256 deactivatedEpoch, , uint256 amount, , ) = sfc
                .delegations(from, toStakerID);
